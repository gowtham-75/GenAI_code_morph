...
from userstories_template import user_stories,extract_json,code_gen
...
# @st.cache_data(show_spinner="Generating JavaCode...")
def generate_code_x(generate_java_code_prompt,value ):
    ...
    final_prompt = PromptTemplate.from_template(generate_java_code_prompt)
    formatted_prompt = final_prompt.format(USER_STORIES=value)    
    response = conversation_chain.predict(input=formatted_prompt)
    full_response = response
    # with st.expander("Length"):
    #     st.write(len(full_response.split()))
    #     st.write(full_response.split())
    ...
    count = 0
    while needs_continuation and count<=5:
        st.spinner(text="In Progress...{count}")
...
def getjavacode(exec_prompt,userstories,model_name):   
    memory_code = ConversationBufferMemory(return_messages=True)    
    conversation_chain = ConversationChain(llm=llm(model_name=model_name), memory=memory_code)
    final_prompt = PromptTemplate.from_template(exec_prompt)
    formatted_prompt = final_prompt.format(user_stories=userstories)  
    response = conversation_chain.predict(input=formatted_prompt)    
    # Initialize the full response
    full_response = response   
    encoding = tiktoken.encoding_for_model("gpt-4o")
...
# @st.cache_data(show_spinner="Generating the java code...")
def getjavacode_new(exec_prompt,userstories,model_name):   
    memory_code = ConversationBufferMemory(return_messages=True)    
    conversation_chain = ConversationChain(llm=llm(model_name=model_name), memory=memory_code)
    final_prompt = PromptTemplate.from_template(exec_prompt)
    
    full_response =""
    for i in range(len(userstories[:5])):
...
def get_code(userstories,input):
    result = getjavacode_new(exec_prompt=code_gen,userstories=userstories,model_name="gpt-4o-mini")
    return result
