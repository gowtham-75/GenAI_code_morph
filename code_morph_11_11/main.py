...
def main():    
    ...
    with userstories_gen:
        ...
        if sql_files_content and enable_option:
            result,final_output = create_user_stories(code=code_text)
            with st.expander("User Stories"):
                st.write(result)
            
            with st.expander("Enhanced User Stories"):
                st.write(final_output)
            if result:
                json_output=get_json(final_output)
    ...
    with view_userstories:
        ...
        if json_output is not None:
            ...
            if st.checkbox('generate the Code'):
                code_result=get_code(json_output,code =code_text )
